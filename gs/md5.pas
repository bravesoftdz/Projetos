...
function GetMD5Str(S: ansiString): ansiString;

function GetMD5KeyStr(S: ansiString): ansiString;
...
begin
	MD5Init(Context);
	FileHandle := CreateFile(pansichar(N), GENERIC_READ, FILE_SHARE_READ or FILE_SHARE_WRITE,
...
function GetMD5Str(S: ansiString): ansiString;
begin
  Result := AnsiUpperCase(MD5Print(MD5String(S)));
end;
...
begin
  ...
  Result := '';                     
  for I := 0 to 15 do
    Result := Result + Char(ansichar(M[I]));
end;
