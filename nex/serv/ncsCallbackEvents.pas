...
type
  ...
  public
    constructor Create(aSessionID: TnxSessionID; aTransp: TnxBaseTransport);
...
begin
  ...
  try DebugMsg('TncCallbaclManager.SessionCallbackTerminated- SessionID: '+ IntToStr(S.FSessionID)); except end;
end;
...
constructor TncSessionCallback.Create(aSessionID: TnxSessionID; aTransp: TnxBaseTransport);
begin
  ...
  DebugMsg('TncSessionCallback.Create - aSessionID: '+ IntToStr(aSessionID));
end;

destructor TncSessionCallback.Destroy;
begin
  DebugMsg('TncSessionCallback.Destroy - aSessionID: '+ IntToStr(FSessionID));
  FEvent.Free;
  inherited;
end;
  ...
  try
    ...
    while (not Terminated) and (not Result) and (Tries<3) do begin
      try
        ...
        Result := True;
        DebugMsg('TncSessionCallback.Envia - OK - SessionID: ' + IntToStr(FSessionID));
      except
      end;
    end;

    if (not Result) and (not Terminated) then begin
      DebugMsg('TncSessionCallback.Envia - FALHOU - SessionID: '+ IntToStr(FSessionID));
...
begin
  DebugMsg('TncSessionCallback.YouMustEnd - SessionID: '+ IntToStr(FSessionID));
  Terminate;
  FEvent.SetEvent;
end;
