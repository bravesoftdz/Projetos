...
begin
  ...
  edBackupDir.Path := ExtractFilePath(ParamStr(0)) + rsCopia;
...
begin
  ...
  for I := 0 to aTabs.Count-1 do begin
    ...
    if not CopyFile(PChar(aTabs[I]), PChar(S), False) then
      Raise ENexCafe.Create(Format(rsErroCopiaArq, [GetLastError, aTabs[I], S]));
    pbR.Repaint;  
    Sleep(100);      
  end;
end;
...
begin
  ...
  try
    try
      qAux.SQL.Text := 'Update Tran set Descr = ' + QuotedStr('') +  ' where ((Descr=' + QuotedStr('')+') or (Descr is null)) AND ' +
...
begin
  ...
  for I := 0 to slArqs.Count-1 do begin
    ...
    if not CopyFile(PChar(slArqs[I]), PChar(S), False) then
      Raise ENexCafe.Create(Format(rsErroCopiaArq, [GetLastError, slArqs[I], S]));
