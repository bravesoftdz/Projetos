...
begin
  OutputDebugString(PChar('VCLFixPack patch installed: ' + S)); // do not localize
end;
{ ---------------------------------------------------------------------------- }
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        DebugLog('GetNonToolWindowPopupParentFix'); // do not localize
        Exit;
      end else
...
begin
  ...
  HookProc(@TOpenCommonDialog.TaskModalDialog, @TCommonDialog_TaskModalDialog, DialogsTaskModalDialogHook);
  DebugLog('FixTaskModalDialog'); // do not localize
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      DebugLog('AppDeActivateZOrderFix'); // do not localize
      Exit;
    end
    else
              ...
              try
                ...
                begin
                  ...
                  begin
                    ...
                    if DebugHook <> 0 then
                    {$WARNINGS ON}
                      OutputDebugString(PChar(Format('The component "%s" of class %s has an endless resize loop', [Ctrl.Name, Ctrl.ClassName]))); // do not localize
                  end;
                end;
              finally
...
begin
  ...
  begin
    DebugLog('ControlResizeFix'); // do not localize
...
begin
  DebugLog('ActionListAVFix'); // do not localize
...
begin
  ...
  RM_GetObjectInstance := RegisterWindowMessage(PChar(Format('ControlOfs%.8X%.8X', [HInst, GetCurrentThreadId]))); // do not localize

  DebugLog('ContextMenuFix'); // do not localize
  { Redirect the original function to the bug fixed version }
...
begin
  ...
  begin
    ...
    begin
      DebugLog('ObjAutoDEPFix'); // do not localize
...
begin
  ...
  begin
    OutputDebugString('Cannot apply Classes.FreeObjectInstance memory leak-fix'); // do not localize
    Exit;
  end;
...
begin
  ...
  begin
    ...
    begin
      DebugLog('AppMinimizeFix'); // do not localize
...
begin
  DebugLog('SysUtilsAbortFix'); // do not localize
...
begin
  ...
  begin
    ...
    begin
      DebugLog('CmdShowMinimizeFix'); // do not localize
      Result := PPointer(P + 4)^;
      Exit;
    end;
    Inc(P);
...
begin
  DebugLog('MDIChildFocusFix'); // do not localize
...
begin
  DebugLog('PageControlPaintingFix'); // do not localize
...
begin
  DebugLog('GridFlickerFix'); // do not localize
  {$IFDEF VCLFIXPACK_DB_SUPPORT}
...
begin
  DebugLog('SpeedButtonGlassFix'); // do not localize
...
begin
  ...
  begin
    ...
    PropInfo := GetPropInfo(TProgressBar, 'MarqueeInterval'); // do not localize
    ...
    DebugLog('VistaProgressBarMarqueeFix'); // do not localize
  end;
end;
...
var
  Info: PPropInfo;
begin
  Info := GetPropInfo(TDBNavigator, 'Flat'); // do not localize
  ...
  begin
    ...
    DebugLog('DBNavigatorFix'); // do not localize
  end;
end;
...
begin
  DebugLog('StringBuilderFix'); // do not localize
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        DebugLog('CancelHintDeadlockFix'); // do not localize
...
begin
  DebugLog('CDSDataConvertFix'); // do not localize
...
function _CompareStringA(Locale: LCID; dwCmpFlags: DWORD; lpString1: PAnsiChar;
  ...
  external kernel32 name 'CompareStringA'; // do not localize
function _CompareStringW(Locale: LCID; dwCmpFlags: DWORD; lpString1: PWideChar;
  ...
  external kernel32 name 'CompareStringW'; // do not localize
...
begin
  ...
  begin
    CompareStringFixRequired := _CompareStringW(LOCALE_USER_DEFAULT, NORM_IGNORECASE, #$FC, 1, 'ue', 2) = CSTR_EQUAL; // do not localize
    if CompareStringFixRequired then
    begin
      OutputDebugStringW('Installing CompareString workaround for Windows Vista'); // do not localize
