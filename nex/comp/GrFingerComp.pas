...
type
  ...
  private
    ...
    procedure pCapInitCallBack(idSensor: PAnsichar; event: GRCAP_STATUS_EVENTS); 
    procedure pCapFingerEvent(idSensor: PAnsichar; event: GRCAP_FINGER_EVENTS); 
    procedure pCapImageEvent(idSensor: PAnsiChar; width: Integer; height: Integer; rawImage: PAnsiChar; res: Integer);
  ...
  public
    ...
    function Match(aTpt: PAnsiChar): Integer;  

    property Tpt: PAnsiChar
      read FTpt;
  ...
  procedure CapInitCallBack(idSensor: PAnsiChar; event: GRCAP_STATUS_EVENTS); stdcall;
  procedure CapFingerEvent(idSensor: PAnsiChar; event: GRCAP_FINGER_EVENTS); stdcall;
  procedure CapImageEvent(idSensor: PAnsiChar; width: Integer; height: Integer; rawImage: PAnsiChar; res: Integer); stdcall;
...
begin
  ...
  inherited;
  FActive := False;
  FTpt := AnsiStrAlloc(GR_MAX_SIZE_TEMPLATE);
  FTptSize := 0;
  FTptQuality := 0;
  FOnFingerprint := nil;
  CriaWndHandle;
...
function TGrFingerComp.Match(aTpt: PAnsiChar): Integer;
begin
  if GrIdentify(aTpt, Result, GR_DEFAULT_CONTEXT)<>GR_Match then
    Result := GR_NOT_MATCH;
end;

procedure TGrFingerComp.pCapFingerEvent(idSensor: PAnsiChar;
  event: GRCAP_FINGER_EVENTS);
begin

end;

procedure TGrFingerComp.pCapImageEvent(idSensor: PAnsiChar; width, height: Integer;
  rawImage: PAnsiChar; res: Integer);
var ret: Integer;  
begin
...
procedure TGrFingerComp.pCapInitCallBack(idSensor: PAnsiChar;
