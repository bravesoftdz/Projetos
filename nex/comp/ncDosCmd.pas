...
type
  ...
  public
    ...
    function SinceBeginning: Cardinal;
    function SinceLastOutput: Cardinal;
  end;
...
begin
  FBeginning := GetTickCount;
  FLastOutput := GetTickCount;
end;
...
function TProcessTimer.SinceBeginning: Cardinal;
begin
  Result := GetTickCount - FBeginning;
end;

function TProcessTimer.SinceLastOutput: Cardinal;
begin
  Result := GetTickCount - FLastOutput;
end;
 ...
 validates the wShowWindow member.}
  ...
  try /// handles of process
    ...
    repeat //main program loop
      ...
      if ((FMaxTimeAfterBeginning < FTimer.SinceBeginning)
...
begin
  ...
  result := Active and (( FTimer.SinceLastOutput > 3000 ) or FThread.FLineBeginned);
end;
...
begin
  result := -1;
  if GetActive then result := FTimer.SinceLastOutput;
end;

function TDosCommand.GetSinceBeginning:integer;
begin
  result := -1;
  if GetActive then result := FTimer.SinceBeginning;
end;
