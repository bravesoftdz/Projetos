...
begin
  Result := IntToStr(logMaq) + sFldDelim(classid_TncAppUrlLogItem) + 
            IntToStr(logSessao) + sFldDelim(classid_TncAppUrlLogItem) +
            GetDTStr(logDH) + sFldDelim(classid_TncAppUrlLogItem) +
            logExe + sFldDelim(classid_TncAppUrlLogItem) +
            logTit + sFldDelim(classid_TncAppUrlLogItem) +
            logUrl + sFldDelim(classid_TncAppUrlLogItem);
end;
...
begin
  Result := GetNextStrDelim(S, classid_TncAppUrlLogItem);
end;
...
begin
  for I := 0 to Count - 1 do  
    Result := Result + Itens[I].AsString + sListaDelim(classid_TncAppUrlLog);
end;
...
procedure TncAppUrlLog.SetString(Value: String);
var S: String;
begin
  Limpa;
  while GetNextListItem(Value, S, classid_TncAppUrlLog) do 
    NewItem.AsString := S;
end;
