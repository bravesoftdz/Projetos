...
type  
  ...
  PmsgInstalaDependEv = ^TmsgInstalaDependEv;
  TmsgInstalaDependEv = packed record
    msgProgresso : Byte;
    msgErro      : Integer;
    msgEtapa     : Byte;
    msgFim       : Boolean;
    msgErroStr   : String[100];
  end;
  ...
  TmsgNFEUpdated = packed record
    msgCCE : Boolean;
    msgUID : TGuid;
  end;
  PmsgNFEUpdated = ^TmsgNFEUpdated;

  TmsgTableUpdated = packed record
    msgTableID : Byte;
  end;
  PmsgTableUpdated = ^TmsgTableUpdated;
...
ncmc_NFEUpdatedEv       = WM_User + 10019;
ncmc_RefreshNFConfigEv  = WM_User + 10020;
ncmc_InstalaDependEv    = WM_User + 10021;
ncmc_TableUpdatedEv     = WM_User + 10022;

ncmc_Primeira    = ncmc_AtualizaObj;
ncmc_Ultima      = ncmc_TableUpdatedEv;
...
begin
  case aMsg of
    ...
    ncmc_NFEUpdatedEv       : Result := 'ncmc_NFEUpdatedEv';
    ncmc_RefreshNFConfigEv  : Result := 'ncmc_NFConfigUpdatedEv';
    ncmc_InstalaDependEv    : Result := 'ncmc_InstalaDependEv';
    ncmc_TableUpdatedEv     : Result := 'ncmc_TableUpdatedEv';
  else
...
begin
  case Msg of
    ...
    ncmc_NFEUpdatedEv : Result := SizeOf(TmsgNFEUpdated);
    ncmc_InstalaDependEv : Result := SizeOf(TmsgInstalaDependEv);
    ncmc_TableUpdatedEv : Result := SizeOf(TmsgTableUpdated);
  else
    Result := 0;
  end;    
end;
...
begin
  Result :=
    ...
    VarIsNull(pbMeioPagto  ) and 
    VarIsNull(pbNexApp     ) and 
    VarIsNull(pbCloudBK    ) and
    VarIsNull(pbBotoes     );
end;
