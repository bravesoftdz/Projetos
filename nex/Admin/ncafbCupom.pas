...
begin
  ...
  if tExpTipoDoc.Value=tiponfe_sat then
    aNomeXML := 'CFe_num'+tExpNumero.AsString+sSufix+'.xml' else
    aNomeXML := 'nfce_serie'+tExpSerie.Value+'_num'+tExpNumero.AsString+sSufix+'.xml';
...
begin
  inherited;
  if not TFrmPeriodo.Create(Self).ObtemPeriodo(I, F, FExpCanc, 'nfcesat') then Exit;
  F := F+1;
  ...
  try
    ...
    while not tExp.Eof do begin
      if (tExpNumero.Value>0) and (tExpStatus.Value>nfestatus_transmitir) and (tExpDataHora.Value<F) and (FExpCanc or (not tExpCancelou.Value)) then begin
        ...
        if tExpContingencia.Value then
...
begin
  ...
  ShowMessage(Dados.CM.InutilizarNFCE(False, aAno, aInicio, aFinal, aJust));
end;
...
begin
  ...
  TFrmXMLs.Create(Self).Mostrar(TabChave.Value, S, TabXMLret.Value, TabXMLdest.Value, TabxmlRetCanc.Value, TabxmlRetInut.Value, False, TabTipoDoc.Value); 
end;
...
begin
  ...
  TFrmXMLs.Create(Self).Mostrar(TabChave.Value, S, TabXMLret.Value, TabXMLdest.Value, TabxmlRetCanc.Value, TabxmlRetInut.Value, True, TabTipoDoc.Value); 
end;
...
begin
  if Dados.tNFConfig.Active then begin
    ...
    if (aUF='') or SameText(aUF, 'SC') then
...
procedure TfbCupom.tExpCalcFields(DataSet: TDataSet);
begin
  inherited;
