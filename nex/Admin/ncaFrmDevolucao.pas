...
type
  TFrmDev = class(TForm)
    ...
    tItensTotLiq: TCurrencyField;
    tItensCustoT: TCurrencyField;
    tItensLucro: TCurrencyField;
    tItensDebDev: TCurrencyField;
    tItensVenDev: TBooleanField;
    MTdebdev: TCurrencyField;
    MTcoddescr: TStringField;
    ...
    procedure cbMostrarCodClick(Sender: TObject);
  private
  ...
  public
    function FazDev(var G: TGUID): Boolean;
...
procedure TFrmDev.ApplyMostrarCod;
begin
  if cbMostrarCod.Checked then
    tvItensdescr.DataBinding.FieldName := 'coddescr' else
    tvItensdescr.DataBinding.FieldName := 'descr';
end;
...
procedure TFrmDev.cbMostrarCodClick(Sender: TObject);
begin
  ApplyMostrarCod;
  if cbMostrarCod.Focused then
    saveFormOptionBool(Self, 'cbMostrarCod', cbMostrarCod.Checked);
end;
...
function TFrmDev.FazDev(var G: TGUID): Boolean;
begin
  ShowModal;
  Result := (ModalResult=mrOk);
  if Result then G := FUID;
end;
...
var I: integer;
begin
  cbMostrarCod.Checked := getFormOptionBool(Self, 'cbMostrarCod');
  ApplyMostrarCod;
...
begin
  ...
  mt.Active := True;
  if FEdit then begin
    TVItensvalordeb.Visible := True;
    TVitensvalordeb.Caption := rsAbatDeb;
    TVitensvalordeb.DataBinding.FieldName := 'debdev';
    TVitensquant.Visible := False;
    TVitensunitario.Visible := False;
    TVitenstotal.Visible := False;
    TVitenstotal_dev.caption := 'Valor Devolvido';
  end else
    TVItensvalordeb.Visible := False;
  ...
  while not tItens.Eof do begin
    ...
    MTcoddescr.Value := MTdescr.Value;
    
    if Dados.tbPro.Locate('ID', tItensProduto.Value, []) then
      MTcoddescr.Value := Dados.tbProCodigo.Value + ' ' + MTcoddescr.Value;
    ...
    begin
      ...
      MTquant_dev.Value := MTquant.Value;
      MTdebdev.Value := tItensDebDev.Value;
