...
resourcestring
  rsNovaDataAss = '1. Nova data de vencimento da assinatura';
  rsRecorrencia = '2. Recorrência';
  rsConfirmacaoFinal = 'Confirmação Final';
  rsMigrar = 'Migrar!';
  rsAvancar = 'Avançar';
  rsMigrouSucesso = 'Parabéns, seu plano foi alterado para PREMIUM com sucesso!';
...
begin
  case Paginas.ActivePageIndex of
    0 : lbPag.Caption := rsNovaDataAss;
    1 : lbPag.Caption := rsRecorrencia;
    2 : 
    begin
      if Boleto then
        lbPag.Caption := '2. '+rsConfirmacaoFinal else
        lbPag.Caption := '3. '+rsConfirmacaoFinal;
    end;    
  end;
  btnVoltar.Enabled := (Paginas.ActivePageIndex>0);
  if Paginas.ActivePageIndex=2 then
    btnAvancar.Caption := rsMigrar else
...
//  ShellStart('http://www.programanex.com.br/info/upgrade-de-plano');
  OpenTrack('info-upgrade-plano', 'FrmMigrarPremium');  
end;
