...
type
  TFrmConfigCodBarBal = class(TFrmBaseOpcaoImgTxtCheckBox)
    ...
    panPrecisao: TLMDSimplePanel;
    btnPrecisao: TcxButton;
    ...
    procedure btnPrecisaoClick(Sender: TObject);
  private
...
uses ncaFrmPri, ncClassesBase, ncaFrmBaseOpcao, ufmImagens, ncaDM,
  ncaFrmConfig_CodBar_Precisao, ncaFrmRecursoPRO;

{$R *.dfm}

resourcestring
  rsRecursoProBal = 'A leitura de código de barras gerado por balanças é um recurso exclusivo para assinantes do plano PRO ou PREMIUM';
...
begin
  ...
  if gConfig.CodBarMaxQtdDig <> FrmConfig_CodBar_Precisao.edMaxQtdDig.Value then Exit;
  if gConfig.CodBarArredondar <> FrmConfig_CodBar_Precisao.edCodBarArredondar.Value then Exit;
  Result := False;
end;

procedure TFrmConfigCodBarBal.btnPrecisaoClick(Sender: TObject);
begin
  inherited;
  FrmConfig_CodBar_Precisao.ShowModal;
end;

procedure TFrmConfigCodBarBal.btnPremiumClick(Sender: TObject);
begin
  inherited;
  TFrmRecursoPro.Create(Self).Mostrar(rsRecursoProBal, 'balanca');
end;
...
begin
  ...
  EnableControls([lbIdent, edIdent, lbTam, edTam, lbDigitos, lbCod, lbInicioCod, edInicioCod, lbFimCod, edFimCod,
                  lbPP, lbInicioPP, edInicioPP, lbFimPP, edFimPP, lbPPValor, edValor, lbCom, edPPDig, lbDigPP],
                  CB.Checked and CB.Enabled);

  panPrecisao.Visible := CB.Enabled and CB.Checked and (edValor.ItemIndex=1);                  
...
procedure TFrmConfigCodBarBal.FormCreate(Sender: TObject);
begin
  inherited;
  FrmConfig_CodBar_Precisao := TFrmConfig_CodBar_Precisao.Create(self);
end;
...
begin
  ...
  FrmConfig_CodBar_Precisao.Load;
  EnableDisable;
end;
...
begin
  ...
  if CB.Checked then begin
    ...
    if VarIsNull(edInicioCod.Value) or (edInicioCod.Value<1) then
    begin
    ...
    gConfig.CodBarMaxQtdDig := FrmConfig_CodBar_Precisao.edMaxQtdDig.Value;
    gConfig.CodBarArredondar := FrmConfig_CodBar_Precisao.edCodBarArredondar.Value;
  end else
