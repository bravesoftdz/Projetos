...
uses
  ...
  dxPScxPivotGridLnk, dxLayoutControlAdapters;


type
  TFrmCadTranspEnt = class(TForm)
    ...
    rbTransp: TcxRadioButton;
    lcTransp: TdxLayoutItem;
    rbEntregador: TcxRadioButton;
    lcEntregador: TdxLayoutItem;
...
var
  FrmCadTranspEnt: TFrmCadTranspEnt;
...
function TFrmCadTranspEnt.Editar(aTab: TDataset; aPai: TWinControl): Boolean;
begin
  result := false;
  Resultado := -1;
...
procedure TFrmCadTranspEnt.cmGravarClick(Sender: TObject);
begin
  Paginas.ActivePageIndex := 0;
  edTelefone.SetFocus;
  edNome.SetFocus;
  MTEmail.Value := edEmail.Text;
  if rbTransp.Checked then
    MTTipoFor.Value := tipofor_transp else
    MTTipoFor.Value := tipofor_entregador;
...
procedure TFrmCadTranspEnt.wmAtualizaStrings(var Msg: TMessage);
begin
{  edBairro.Properties.Items.Text := slBairros.Text;
  edCidade.Properties.Items.Text := slCidades.Text;}
end;

procedure TFrmCadTranspEnt.AfterEditarEnd(Sender: TObject);
begin
  edEmail.SetFocus;
end;

procedure TFrmCadTranspEnt.btnNenhumClick(Sender: TObject);
var
  Dia, H : Integer;
  SB : TLmdSpeedButton;
begin
...
procedure TFrmCadTranspEnt.btnTodosClick(Sender: TObject);
var
  Dia, H : Integer;
  SB : TLmdSpeedButton;
begin
...
procedure TFrmCadTranspEnt.cmCancelarClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmCadTranspEnt.FMgrGetTabSheet(Sender: TObject;
...
procedure TFrmCadTranspEnt.FMgrPageControlChange(Sender: TObject);
begin
  if paginas.ActivePage = TsTran then begin
    ...
    if FMgr.FormByClass(TfbTran)=nil then begin
      ...
      try
        FMgr.Mostrar(TfbTran, ftran_entrega, MTID.Value);
...
procedure TFrmCadTranspEnt.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  FMgr.Clear;
  Action := caFree;
end;

procedure TFrmCadTranspEnt.FormCreate(Sender: TObject);
const
...
procedure TFrmCadTranspEnt.FormShow(Sender: TObject);
begin
  if MTTipoFor.Value=tipofor_transp then
    rbTransp.Checked := True else
    rbEntregador.Checked := True;
...
procedure TFrmCadTranspEnt.hora10Click(Sender: TObject);
begin
...
procedure TFrmCadTranspEnt.hora10MouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
...
procedure TFrmCadTranspEnt.InspNomeIsentoPropertiesGetEditingProperties(
  Sender: TcxCustomEditorRowProperties; ARecordIndex: Integer;
  var AProperties: TcxCustomEditProperties);
begin
...
procedure TFrmCadTranspEnt.InspNomeTAPropertiesGetEditingProperties(
  Sender: TcxCustomEditorRowProperties; ARecordIndex: Integer;
  var AProperties: TcxCustomEditProperties);
begin
...
procedure TFrmCadTranspEnt.NBLinkClick(Sender: TObject; ALink: TdxNavBarItemLink);
begin
  Paginas.ActivePageIndex := ALink.Index;
end;

function TFrmCadTranspEnt.Novo(aTab: TDataset; aPai: TwinControl): Integer;
begin
  if aTab=nil then aTab := Dados.tbCli;
  ...
  FNovo := True;
  MT.Insert;
  MTTipoFor.Value := tipofor_transp;
...
procedure TFrmCadTranspEnt.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then Key := #0;
end;

procedure TFrmCadTranspEnt.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmCadTranspEnt.DSStateChange(Sender: TObject);
begin
  RefreshBtn;
end;

procedure TFrmCadTranspEnt.dxBarLargeButton1Click(Sender: TObject);
begin
  CP.Print(False, nil)
end;

procedure TFrmCadTranspEnt.MTAfterInsert(DataSet: TDataSet);
begin
  FRgAnt := '';
end;

procedure TFrmCadTranspEnt.MTAfterEdit(DataSet: TDataSet);
begin
  //
end;
