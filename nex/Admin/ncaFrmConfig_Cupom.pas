...
type
  TFrmConfig_Cupom = class(TFrmBaseOpcao)
    ...
    MTsat_esq: TByteField;
    lcsat_esq: TdxLayoutItem;
    edsat_esq: TcxDBImageComboBox;
...
var
  FrmConfig_Cupom: TFrmConfig_Cupom;
...
function TFrmConfig_Cupom.Alterou: Boolean;
begin
  Result := True;
  with Dados do begin
    ...
    if edsat_esq.EditValue <> tNFConfigsat_esq.AsVariant then Exit;
  end;
  Result := False;
end;

function TFrmConfig_Cupom.ESAT: Boolean;
begin
  Result := (not VarIsNull(edEstado.EditValue)) and SameText(edEstado.EditValue, 'SP');
end;

procedure TFrmConfig_Cupom.Atualiza;
var 
  aSAT: Boolean;
begin
...
procedure TFrmConfig_Cupom.AtualizaMun;
begin
  with Dados do 
...
procedure TFrmConfig_Cupom.btnAvancadasClick(Sender: TObject);
begin
  inherited;
  lcgr_Avancado.Visible := btnAvancadas.Down;
end;

procedure TFrmConfig_Cupom.btnInstalaDependClick(Sender: TObject);
begin
  inherited;
...
procedure TFrmConfig_Cupom.btnOkClick(Sender: TObject);
begin
...
procedure TFrmConfig_Cupom.btnPremiumClick(Sender: TObject);
begin
  inherited;
  TFrmRecursoPremium.Create(Self).Mostrar(rsNFCePremium, 'nfce');
end;

procedure TFrmConfig_Cupom.edCodMunPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var 
  aUF, 
  aCod: String;  
...
procedure TFrmConfig_Cupom.edCodMunPropertiesChange(Sender: TObject);
begin
  inherited;
  AtualizaMun;
end;

procedure TFrmConfig_Cupom.edCRTPropertiesChange(Sender: TObject);
begin
  inherited;
...
procedure TFrmConfig_Cupom.edEmitirNFCePropertiesChange(Sender: TObject);
begin
  inherited;
  Atualiza;
  EnableDisable;
...
procedure TFrmConfig_Cupom.edEstadoPropertiesChange(Sender: TObject);
begin
  inherited;
  EnableDisable;
  Atualiza;
...
procedure TFrmConfig_Cupom.edMostrarNCMPropertiesChange(Sender: TObject);
begin
  inherited;
  Atualiza;
end;
     
procedure TFrmConfig_Cupom.edMostrarSitTribPropertiesChange(Sender: TObject);
begin
  inherited;
  Atualiza;
end;

procedure TFrmConfig_Cupom.edNomeDLLSatPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
  inherited;
...
procedure TFrmConfig_Cupom.edsat_modeloPropertiesChange(Sender: TObject);
begin
  inherited;
  if edsat_modelo.Focused then Atualiza;
end;

procedure TFrmConfig_Cupom.edsat_modeloPropertiesCloseUp(Sender: TObject);
begin
  inherited;
  Atualiza;
end;

procedure TFrmConfig_Cupom.edTipoCertPropertiesChange(Sender: TObject);
begin
  inherited;
  Atualiza;
end;

procedure TFrmConfig_Cupom.EnableDisable;
var aEnable: Boolean;
begin
  lcPIN.Visible := False; //(edTipoCert.ItemIndex=1);
...
procedure TFrmConfig_Cupom.FormCreate(Sender: TObject);
begin
  inherited;
  FSignACCNPJ := '';
...
procedure TFrmConfig_Cupom.FormDestroy(Sender: TObject);
begin
  inherited;
  if Assigned(FFrmNCM) then gNCMPesqList.ReleaseFrm(FFrmNCM);
end;

procedure TFrmConfig_Cupom.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TFrmConfig_Cupom.GerarSignACClick(Sender: TObject);
var 
  S: String;
begin
  inherited;
...
procedure TFrmConfig_Cupom.Ler;
begin
  inherited;
  MT.Active := False;
  MT.Active := True;
...
class procedure TFrmConfig_Cupom.Mostrar(aAtivar: Boolean);
begin
  inherited;
  ...
  if gConfig.IsPremium and (not gConfig.Pro) then 
    with TFrmConfig_Cupom.Create(nil) do begin
      Ativar := aAtivar;                                  
      ShowModal;
    end;  
end;

function TFrmConfig_Cupom.NumItens: Integer;
begin
  Result := 3;
end;

procedure TFrmConfig_Cupom.PaginasChange(Sender: TObject);
begin
  inherited;
...
procedure TFrmConfig_Cupom.PaginasDrawTabEx(AControl: TcxCustomTabControl;
...
procedure TFrmConfig_Cupom.Renumera;
begin
//  RenumCB(edEmitirNFCe, 0);
//  RenumLB(lbCRT, 2);
end;

procedure TFrmConfig_Cupom.Salvar;
var 
...
begin
  ...
  with Dados do begin
    ...
    if tNFConfigEmitirNFCe.Value and (not aEmissaoAntes) then begin 
      ...
      Dados.EnviaEmailAtivacaoNF(False, '', '');
    end else
    if tNFConfigEmitirNFCe.Value then begin
      ...
      if (aRazaoAnt>'') or (aCNPJAnt>'') then
        Dados.EnviaEmailAtivacaoNF(False, aCNPJAnt, aRazaoAnt);
    end;    
    
  end;
end;

procedure TFrmConfig_Cupom.SetEditModelo(const Value: Boolean);
begin
  Paginas.ActivePageIndex := 3;
  FEditModelo := Value;
end;

procedure TFrmConfig_Cupom.Timer1Timer(Sender: TObject);
begin
  inherited;
...
procedure TFrmConfig_Cupom.Valida;
begin
...
procedure TFrmConfig_Cupom.wmAtualizaDireitosConfig(var Msg: TMessage);
var I : Integer;
  B : Boolean;
begin
