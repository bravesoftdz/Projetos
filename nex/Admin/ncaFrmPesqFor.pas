...
begin
  ...
  aID := TFrmCadFornecedor.Create(Self).Novo(Tab, nil, FDMDanfe);
  if aID>0 then begin
    Tab.Locate('ID', aID, []);
    FUpdating := True;
    try
...
begin
  ...
  try
    ...
    Tab.SetRange([True], [True]);
    if (aID>0) then begin
      if Tab.Locate('ID', aID, []) then begin
  ...
  if ModalResult=mrOk then begin
    if (edBusca.Text>'') and (not Tab.IsEmpty) then begin
      ...
      if (aDMDanfe<>nil) and (SoDig(TabCPF.Value)<>SoDig(aDMDanfe.mtEmitCNPJ.Value)) then begin
        Tab.Edit;
        TabCPF.Value := SoDig(aDMDanfe.mtEmitCNPJ.Value);
        Tab.Post;
      end;
    end else begin
      aID := 0;
