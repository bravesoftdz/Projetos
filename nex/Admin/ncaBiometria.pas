...
uses
  ...
  LMDControl, cxPCdxBarPopupMenu, cxGraphics, cxLookAndFeels, frxClass,
  dxBarBuiltInMenu;
...
//    FActiveH : HWND;
  ...
  public
    procedure SearchTPT(aTpt: PAnsiChar; aTptSize: Integer; aQld: Byte);
...
resourcestring
  SVocêNãoSelecionouUmClienteParaEs = 'Você não selecionou um cliente para essa digital';
...
begin
  if idCli=0 then 
    Raise ENexCafe.Create(SVocêNãoSelecionouUmClienteParaEs);
...
var 
  ...
  S : TMemoryStream;
begin
  T := AnsiStrAlloc(GR_MAX_SIZE_TEMPLATE);
  S := TMemoryStream.Create;
  try
...
procedure TFrmBio.SearchTPT(aTPT: PAnsiChar; aTPTSize: Integer; aQld: Byte);
begin
