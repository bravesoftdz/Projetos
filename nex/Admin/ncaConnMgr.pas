...
uses SysUtils, Classes, SyncObjs, Windows, Messages, madTypes, madTools, nxtwWinsockTransport, idHttp;
...
type
  ...
  private
    ...
    procedure wmUpdate(window: HWND;  msg: cardinal; wParam, lParam: NativeInt; var result: NativeInt);
...
begin
  ...
  try
    ...
    try
      ...
      try
        NextTry := 0;
        while (Result='') and (not gConnmgrStopRetry) and (tries<5) do begin
          if NextTry>0 then Sleep(500);
          if GetTickCount>=NextTry then begin
            try 
              GetServerNames(sl, 1500); 
            except 
              on e:exception do DebugMsg('GetLocalServerName Exception: '+E.message);
            end;
            if sl.Count>0 then begin
              ...
              Inc(Tries);
              NextTry := GetTickCount+(5000*tries);
...
begin
  ...
  FLastConnOk := False;
  LoadAuto;
  AddMsgHandler(wmUpdate, wm_user, 0);
...
procedure TConnMgr.wmUpdate(window: HWND; msg: cardinal; wParam, lParam: NativeInt;
