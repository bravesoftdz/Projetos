...
type
  ...
  private
    ...
    FOnChangeDesconto : TNotifyEvent;
    ...
    function TotalSemDesc: Currency;
  ...
  public
    ...
    procedure LoadData(aPagEsp: TncPagEspecies; aProdutos, aTaxIncluir, aFrete, aDesconto: Currency; aDescPerc: Double; aDescPorPerc: Boolean); 
  
    function Editar(aNovo: Boolean; aPagEsp: TncPagEspecies; aValidator: TncDebCredValidator; aProdutos, aTaxIncluir, aFrete: Currency; var aDesconto: Currency; var aDescPerc: Double; var aDescPorPerc: Boolean; const aPodeSalvar: Boolean = True; const aPodeDebito: Boolean = True; const aPodeCredito: Boolean = True): Boolean;

    property FontSize: Integer
      read FFontSize write FFontSize;  

    property OnChangeDesconto: TNotifyEvent
      read FOnChangeDesconto write FOnChangeDesconto;    
...
resourcestring
  ...
  rsErroDesc   = 'Desconto não pode ser maior que o total a pagar';
  rsErroPagouMais = 'Valor pago não pode ser maior que o valor a pagar';
  rsErroCredMais = 'O valor pago com crédito não pode ser maior que o crédito disponivel do cliente';
...
resourcestring
  rsRecursoProDeb = 'O controle de débitos é um recurso exclusivo para assinantes do plano PRO';
  rsRecursoProTroco = 'A opção de creditar o troco na conta do cliente é um recurso exclusivo para assinantes do plano PRO';
...
begin
  lbTotalSemDesc.Caption := CurrencyToStr(TotalSemDesc);
  
  Result := FProdutos - FDesconto + FTaxIncluir + FFrete;
end;
...
function TFrmPagEspecie.TotalSemDesc: Currency;
begin
  Result := FProdutos + FTaxIncluir + FFrete;
end;
...
function TFrmPagEspecie.Editar(aNovo: Boolean; aPagEsp: TncPagEspecies; aValidator: TncDebCredValidator; aProdutos, aTaxIncluir, aFrete: Currency; var aDesconto: Currency; var aDescPerc: Double; var aDescPorPerc: Boolean; const aPodeSalvar: Boolean = True; const aPodeDebito: Boolean = True; const aPodeCredito: Boolean = True): Boolean;
begin
  ...
  LoadData(aPagEsp, aProdutos, aTaxIncluir, aFrete, aDesconto, aDescPerc, aDescPorPerc);
  ShowModal;
...
//  if not panDesconto.Visible then Exit;
  ...
  Desc := Desconto;
  if TFrmEditDesc.Create(Self).Editar(FProdutos, FFrete, FTaxIncluir ,Desc, aDescPerc, aDescPorPerc) then begin
    ...
    if Assigned(FOnChangeDesconto) then FOnChangeDesconto(Self);
  end;
end;
...
procedure TFrmPagEspecie.LoadData(aPagEsp: TncPagEspecies; aProdutos, aTaxIncluir, aFrete, aDesconto: Currency; aDescPerc: Double; aDescPorPerc: Boolean);
begin
