...
//  for J := 1 to 2 do 
  for I := 0 to SL.Count-1 do begin
    ...
    tProd.Append;
    s2 := ProxCampo;
    if StrToIntDef(SoDig2(s2), 0)=0 then begin
      Inc(Cod);
      tProdCodigo.Value := CodStr(Cod);
    end else
      tProdCodigo.Value := s2;
    tProdDescricao.Value := ProxCampo;
    tProdPreco.Value := StrToCurrency(ProxCampo);
    tProdCustoUnitario.Value := StrToCurrency(ProxCampo);
    tProdPodeAlterarPreco.Value := (Copy(ProxCampo, 1, 1)='S');
    tProdPermiteVendaFracionada.Value := (Copy(ProxCampo, 1, 1)='S');
    ProxCampo;
    ProxCampo;
    tProdNaoControlaEstoque.Value := (Copy(ProxCampo, 1, 1)='N');
    tProdCategoria.Value := ProxCampo;
    tProdUnid.Value := ProxCampo;
    tProdFornecedor.Value := Fornecedor(ProxCampo, '');
    tProd.Post;
  end;
end;
