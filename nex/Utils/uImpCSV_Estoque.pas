...
//  for J := 1 to 2 do 
  for I := 1 to SL.Count-1 do begin
    ...
    S := SL[I];
    tProd.Append;
    tProdCodigo.Value := IntToStr(I);
    tProdDescricao.Value := Trim(ProxCampo);
    s2 := Trim(ProxCampo);
    if s2>'' then
      tProdDescricao.Value := tProdDescricao.Value + ' ' + s2;
    
    Qtd := StrToIntDef(ProxCampo, 0);
