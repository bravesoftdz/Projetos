...
uses unxInst_Folder_en, madKernel, ncFirewall;
  ...
  case Paginas.ActivePageIndex of
    2 : begin
      if not cbConcordo.Checked then begin
        Beep;
        ShowMessage('You must agree with service the terms');
        cbConcordo.SetFocus;
        Exit;
      end;
      ...
      try  
        ...
        if rbLocal.Checked or cbServ.Checked then 
          addApplicationToFirewall('Nextar Server', fullexe(1));
        addApplicationToFirewall('Nextar Admin', fullexe(2));
      except
      end;
    end;
...
begin
  case Paginas.ActivePageIndex of
    2 : btnAvancar.Caption := 'Install!';
    3 : btnAvancar.Caption := 'Finish';
  else
    btnAvancar.Caption := '&Next >';
  end;
