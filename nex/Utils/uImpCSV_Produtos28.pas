...
//  for J := 1 to 2 do 
  for I := 1 to SL.Count-1 do begin
    ...
    S := SL[I];   
    s2 := ProxCampo;
    if (not SameText(s2, 'Produto:')) and (s2>'') then begin
      tProd.Append;
      tProdDescricao.Value := s2;
      tProdUnid.Value := ProxCampo;
      tProdPreco.Value := StrToCurrency(ProxCampo);
      Custo := 0; C := 0;
      for J := 1 to 7 do begin
        C := StrToCurrency(ProxCampo);
        if (C>0) and ((Custo=0) or (C<Custo)) then Custo := C;
      end;
      if Custo>0 then tProdCustoUnitario.Value := Custo;
