...
{    if J=2 then begin
    ...
    ProxCampo;
    sDoc := Trim(ProxCampo);
    if Length(SoDig(sDoc))=11 then
      tCliCPF.Value := SoDig(sDoc) 
    else
    if Pos('RG', UpperCase(sDoc))>0 then
      tCliRG.Value := sDoc else
      tCliObs.Value := Trim(sDoc);
