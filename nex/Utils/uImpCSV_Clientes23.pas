...
//  for J := 1 to 2 do 
  for I := 1 to SL.Count-1 do begin
    ...
    ProxCampo; ProxCampo;
    AddCampoObs('Data da visita: ', ProxCampo);
    ProxCampo;
    with Split(ProxCampo, '/') do
    try
      if Count>0 then
        tCliTelefone.Value := Strings[0];
      if Count>1 then
        tCliCelular.Value := Strings[1];
      if Count>2 then
        AddCampoObs('', Strings[2]);  
    finally
      Free;
    end;
    ProxN(4);
