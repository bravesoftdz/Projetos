...
uses
  ...
  ncaFrmConfigComissao, ncaFrmObs;
...
begin
  ...
  if not FNovo then begin
    tProdFor.SetRange([tbProID.Value], [tbProID.Value]);
    L := TList.Create;
    try
      while not tProdFor.Eof do begin
      ...
      for I := 0 to L.Count-1 do
        if not FListaFor.AchaFor(Integer(L[I])) then 
          if tProdFor.FindKey([tbProID.Value, Integer(L[I])]) then 
            tProdFor.Delete;
    finally
      L.Free;
    end;
  ...
  if Valido then begin
    if tProdFor.FindKey([tbProID.Value, Fornecedor]) then
      tProdFor.Edit else
      tProdFor.Insert;
    tProdForProduto.Value := tbProID.Value;
...
begin
  ...
  if Dados.NFCeAtivo and (Trim(edUnid.Text)='') then begin
    Paginas.ActivePage := tsDados;
    edUnid.SetFocus;
  ...
  with Dados do begin
    ...
    if CorrigeEstoque(tbProID.Value, FEstoqueAnt, edAtual.Value) then Tab.Refresh;
