...
begin
  if SameText('msgfiscal', Sender.Name) then
    Sender.Visible := Dados.NFCeAtivo;
end;
...
begin
    ...
    if tTranDesconto.Value>0 then begin
      TotProd := FTaxDescr.TemTaxAdd;
      if TotProd then
        AddPag(strTotProdutos, tTranTotal.Value, False) else
        AddPag(strSubTotal, tTranTotal.Value, False);
