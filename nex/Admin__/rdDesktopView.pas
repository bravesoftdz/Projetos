...
resourcestring
  SDesktop = ' - Desktop ';
  SLendoTelaInicialAguarde = 'Lendo tela inicial. Aguarde ...';
  SSessãoFinalizadaPelaMáquinaClien = ' - Sessão finalizada pela máquina cliente';
  SSessãoFinalizadaPelaMáquinaClien_1 = 'Sessão finalizada pela máquina cliente.';
  SDesconectado = ' - Desconectado';
  SFimDeSessão = 'Fim de sessão.';
  SMáquina = 'Máquina ';
  SSairDaTelaCheia = 'Sair da tela cheia';
  SIrParaTelaCheia = 'Ir para Tela Cheia';
  SExibirFundoDeTela = 'Exibir fundo de tela';
  SRemoverFundoDeTela = 'Remover fundo de tela';
...
function checkControl:string;
  begin
  {$IFNDEF RtcViewer}
  Result:='Control'; // do not localize
  {$ELSE}
  Result:='View'; // do not localize
  {$ENDIF}
  end;
...
procedure TrdDesktopViewer.Mostrar;
begin
  Show;
  BringToFront;
...
begin
  if (myUI.ScreenWidth>Width) or (myUI.ScreenHeight>Height) then
    pImage.Align := alClient else
    pImage.Align := alNone;

  if pImage.Align=alNone then begin
    pImage.Width := myUI.ScreenWidth;
    pImage.Height := myUI.ScreenHeight;
...
procedure TrdDesktopViewer.myUIOpen(Sender: TRtcPDesktopControlUI);
  ...
  Caption:=myUI.UserName+SDesktop+checkControl;
  sStatus.Font.Color:=clWhite;
  sStatus.Caption:=SLendoTelaInicialAguarde;
...
procedure TrdDesktopViewer.myUIClose(Sender: TRtcPDesktopControlUI);
  ...
  Caption:=Caption+SSessãoFinalizadaPelaMáquinaClien;
  sStatus.Font.Color:=clRed;
  sStatus.Caption:=SSessãoFinalizadaPelaMáquinaClien_1;
  sStatus.Visible:=True;
...
procedure TrdDesktopViewer.myUIError(Sender: TRtcPDesktopControlUI);
  ...
  Caption:=Caption+SDesconectado;
  sStatus.Font.Color:=clRed;
  sStatus.Caption:=SFimDeSessão;
  sStatus.Visible:=True;
...
procedure TrdDesktopViewer.myUIData(Sender: TRtcPDesktopControlUI);
  ...
  if sStatus.Visible and UI.HaveScreen then
    begin
    Caption:=SMáquina+myUI.UserName; //+' - Desktop '+checkControl;
    sStatus.Visible:=False;
    pImage.Constraints.maxHeight := myUI.ScreenHeight;
    pImage.Constraints.maxWidth := myUI.ScreenWidth;
    WindowState:=wsNormal;
    if myUI.ScreenWidth<Screen.Width then
      ClientWidth:=myUI.ScreenWidth
    else
...
procedure TrdDesktopViewer.ScrollMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
  ...
  if not (panSettings.Visible or panOptions.Visible) then
    ...
    if btnCycle.Down then btnCycle.Hint:=SSairDaTelaCheia 
    else btnCycle.Hint:=SIrParaTelaCheia; 
...
procedure TrdDesktopViewer.pImageMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
  begin
  if not (LMouseDown or RMouseDown) then
...
procedure TrdDesktopViewer.btnWallpaperClick(Sender: TObject);
  ...
  if btnWallpaper.Down then
    begin
    btnWallpaper.Hint:=SExibirFundoDeTela;
    UI.Send_HideDesktop;
    end
  else
    begin
    btnWallpaper.Hint:=SRemoverFundoDeTela;
    UI.Send_ShowDesktop;
    end;
  end;
