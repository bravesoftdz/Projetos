...
uses ncaDM, ncaFrmDoc, ncGuidUtils, ncaFrmPri, ncClassesBase, ncaDMComp,
  ...
  ncDMdanfe_NFCE, ncDMdanfe_SAT, uNexTransResourceStrings_PT;
...
begin
  if not (FrmPri.FM.FormAtivo is TfbProdutos) then 
    FrmPri.FM.Mostrar(TfbProdutos);
    
  Result := TfbProdutos(FrmPri.FM.FormAtivo);
end;
...
begin
  ...
  try
    panPersonalizar.Visible := StringToBool(slConfig.Values['docedit']) and (FTipo<4);
    tDoc.Active := True;
