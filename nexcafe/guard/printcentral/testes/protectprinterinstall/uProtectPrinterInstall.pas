...
Const
  ...
  kNexCafe5PrintCommand = 'nexAP.exe';
...
uses
   uDirUtils, uZipUtils, uCommonProc, uDisplayUtils, uVersionInfo, ncgPrintInstall;
...
begin
    ...
    try
        ...
        if r.openKey(kKey3+result, true) then begin
            ...
            r.WriteInteger('ShowWindow',kRedirectedPortShowWindowHidden); 
...
Function SetAccessPrivilege(Const PrivilegeStr : String; State : Integer):boolean;
Var
  hToken : THandle;
  aluid : TLargeInteger;
  cbPrevTP : DWORD;
...
function SaveRegKey(k,fn:string):boolean;
var
    filen, param, dir : string;
begin
    filen := 'regedit.exe';
    param := ' /ES '+fn+' "'+k+'"';
    dir   := 'c:\system32';
    ShellExecute(0, 'open', pchar(filen), pchar(param), PChar(dir), SW_HIDE);
End;

function RestoreRegKey(k,fn:string):boolean;
var
    filen, param, dir : string;
begin
    filen := 'regedit.exe';
...
{
function SaveRegKey(rk:integer;k,fn:string):boolean;
Var
  R: TRegistry;
begin
  result := false;
  ...
  Try
    ...
    Begin
      R.CloseKey;
      SetAccessPrivilege('SeBackupPrivilege', SE_PRIVILEGE_ENABLED); //See below for this function
      If FileExists(fn) Then
        deletefile(fn);
      result := R.SaveKey(k,fn);
    End;
  Finally
    R.Free;
  End;
...
function RestoreRegKey(rk:integer;k,fn:string):boolean;
Var
  R: TRegistry;
begin
  result := false;
  ...
  try
    ...
    if SetAccessPrivilege('SeRestorePrivilege',SE_PRIVILEGE_ENABLED) then
        //This will overwtite all keys and values if they already exist
        result := R.RestoreKey(k,fn);
        //result := R.LoadKey(k,fn);
  Finally
    R.Free;
  End;
End;   }
...
begin
    cxMemo1.clear;
    SaveRegKey( 'HKEY_CURRENT_USER\Printers', 'C:\regImpre\1.reg');
    SaveRegKey( 'HKEY_CURRENT_USER\Software\Microsoft\Windows NT\CurrentVersion\Devices', 'C:\regImpre\2.reg');
    SaveRegKey( 'HKEY_CURRENT_USER\Software\Microsoft\Windows NT\CurrentVersion\PrinterPorts', 'C:\regImpre\3.reg');
    SaveRegKey( 'HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Print', 'C:\regImpre\4.reg');
...
begin
    ...
    RestoreRegKey( 'HKEY_CURRENT_USER\Printers', 'C:\regImpre\1.reg');
    RestoreRegKey( 'HKEY_CURRENT_USER\Software\Microsoft\Windows NT\CurrentVersion\PrinterPorts', 'C:\regImpre\3.reg');
    RestoreRegKey( 'HKEY_CURRENT_USER\Software\Microsoft\Windows NT\CurrentVersion\Devices', 'C:\regImpre\2.reg');
    RestoreRegKey( 'HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Print', 'C:\regImpre\4.reg');
