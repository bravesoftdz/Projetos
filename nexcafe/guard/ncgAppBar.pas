...
type
  ...
  { Internal implementation state variables }
    ...
    // We need a member variable which tracks whether our autohide windo
    // is visible or not
  ...
  { AppBar-specific helper functions }
    ...
    property DiscardFull: Boolean
      read FDiscardFull write FDiscardFull;                               
  ...
  { Public member functions }
    ...
    function GetDefDockSize: Integer; virtual;

    // Constructs an AppBar
    constructor Create (Owner : TComponent); override;

    procedure ResetSystemKnowledge;
    procedure SendBottom;
    ...
    property FullScreenAppOpen: Boolean
      read FbFullScreenAppOpen;
...
function TAppBar.GetDefDockSize: Integer;
begin
  Result := AB_DEF_DOCK_SIZE;
end;
...
// TAppBar.SetEdge ////////////////////////////////////////////////////////////
procedure TAppBar.SendBottom;
begin
  SetWindowPos(Handle, HWND_BOTTOM, 0, 0, 0, 0, SWP_NOMOVE or SWP_NOACTIVATE or SWP_NOSIZE);
end;
...
begin
  ...
  if FABS.bAlwaysOnTop then begin
    ...
    hWnd := HWND_TOPMOST;
    if FbFullScreenAppOpen {and (not FDiscardFull)} then
...
//  if not FDiscardFull then begin
...
begin
  if FullScreenAppOpen or FrmPri.FullAppOpen then Exit;
...
begin
  ...
  FABS.szDockSize.cx     := GetDefDockSize;
  FABS.szDockSize.cy     := GetDefDockSize;
  FABS.rcFloat.Left      := 0;
  FABS.rcFloat.Top       := 0;
