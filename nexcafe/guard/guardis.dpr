...
const CServiceName  = 'NexGuardIS';
      CServiceDescr = 'NexCafé IS';
...
begin
  ...
  if dir.inject then
       answer :=   InjectLibrarySession(dir.session, false, dir.dllname, dir.timeOut)
  else answer := UninjectLibrarySession(dir.session, false, dir.dllname, dir.timeOut);
end;
...
procedure ShowMsg(aMsg, aCaption: pchar; flags: Cardinal);
begin
  if not IsTextEqual(ParamStr(2), 'SILENT') and
     not IsTextEqual(ParamStr(3), 'adcionarbytesguardis') then
    MessageBox(0, aMsg, aCaption, flags);
end;
  ...
  if c1 <> 0 then begin
    ...
    if c2 <> 0 then begin
      ...
      if (not b1) and (ss.dwCurrentState = SERVICE_RUNNING) then begin
        ...
        if IsTextEqual('REMOVER', ParamStr(1)) then
        if ControlService(c2, SERVICE_CONTROL_STOP, ss) then begin
          if DeleteService(c2) then
               ShowMsg('o serviço foi removido de novo', 'informação...', MB_ICONINFORMATION)
          else ShowMsg('o serviço foi parado, mas a tentativa de remove-lo falhou', 'atenção...',     MB_ICONWARNING);
        end else
          MessageBox(0, 'falha ao tentar parar serviço', 'atenção...', MB_ICONWARNING);
      end else begin
        if b1 then
          ...
          if ChangeServiceConfigA(c2, CServiceType, CServiceStart, SERVICE_ERROR_NORMAL,
               ...
               ShowMsg('correction of service parameters succeeded', 'information...', MB_ICONINFORMATION)
          else ShowMsg('correction of service parameters failed',    'warning...',     MB_ICONWARNING);
          
        if ss.dwCurrentState <> SERVICE_RUNNING then
          // our service was installed, but not running, so we start it
          if IsTextEqual('INSTALAR', ParamStr(1)) then begin
            if StartServiceA(c2, 0, nil) then
              ShowMsg('the service was restarted', 'information...', MB_ICONINFORMATION) else
              ShowMsg('restarting failed',         'warning...',     MB_ICONWARNING);
          end else 
          if IsTextEqual('REMOVER', ParamStr(1)) then begin
            if DeleteService(c2) then
              ShowMsg('the service is removed again',                'information...', MB_ICONINFORMATION) else
              ShowMsg('the service is stopped, but removing failed', 'warning...',     MB_ICONWARNING);
          end;    
      end;
      CloseServiceHandle(c2);
    end else 
    if IsTextEqual(ParamStr(1), 'INSTALAR') then begin
      ...
      if c2 <> 0 then begin
        ...
        if not AddAccessForEveryone(c2, SERVICE_ALL_ACCESS or _DELETE) then
          ShowMsg('access manipulation didn''t work', 'warning...', MB_ICONWARNING);
        ...
        if StartServiceA(c2, 0, nil) then begin
          ...
          if ss.dwCurrentState = SERVICE_RUNNING then
               ShowMsg('the service is installed now',      'information...', MB_ICONINFORMATION)
          else ShowMsg('installation failed (ipc failure)', 'warning...',     MB_ICONWARNING);
        end else
          ShowMsg('installation succeeded, but starting failed', 'warning...', MB_ICONWARNING);
        CloseServiceHandle(c2);
      end else
        ShowMsg('you don''t have enough privileges', 'sorry...', MB_ICONWARNING);
    end;
    CloseServiceHandle(c1);
  end else
    ShowMsg('you don''t have enough privileges', 'sorry...', MB_ICONWARNING);
...
begin
{  NomeArqDebug := 'debug_guardis.txt';
  DebugAtivo := True;}
