...
uses windows, sysutils, classes, forms, uPrinterConstsAndTypes,
     uPrintException, printers, winspool, uCommonProc, registry, strutils,
     dialogs, math, JclDateTime, IniFiles;

function  PrinterSupportsPostscript(aPrinterName:string): Boolean;
function  CapturePrinterInfo8(Sender: TForm; APrinterName: PChar; ADestStream: TStream; aForceA4:boolean):string;
      ...
      try
        ...
        begin
          ...
          try
            ...
            if GetPrinter(HPrinter, 8, PI8, BytesNeeded, @BytesNeeded) then begin

              //PrinterProperties(0, hPrinter);
              if DocumentProperties(Sender.handle, hPrinter, APrinterName, PI8.pDevMode^,
                   PI8.pDevMode^, DM_IN_PROMPT or DM_IN_BUFFER or DM_OUT_BUFFER)=IDOK then begin

                   if aForceA4 then begin
                      // A4 210 x 297 mm', 'A4', 2100, 2970, DMPAPER_A4
                      PI8.pDevMode^.dmPaperSize := DMPAPER_A4;
                      PI8.pDevMode^.dmPaperLength := 2970;
                      PI8.pDevMode^.dmPaperWidth  := 2100;
                   end;

                   ADestStream.Write(PChar(PI8)[InfoSize], BytesNeeded - InfoSize);
