...
begin
  ...
  for I := 0 to SL.Count-1 do begin
    ...
    if pos('CÓDIGO',S) > 0 then begin
      tCli.Insert;
      tCliID.AsString := SoDig(Copy(S, 9, 7));
      tCliUsername.Value := tCliID.AsString;
      tCliNome.Value := Trim(Copy(S, 22, 50));
      S := SL[I+1];
      tCliEndereco.Value := Trim(Copy(S, 11, 50));
      S := SL[I+2];
      tCliCidade.Value := Trim(Copy(S, 23, 50));
      S := SL[I+3];
      tCliTelefone.Value := Trim(Copy(S, 11, 30));
      tCliRG.Value := Trim(Copy(S, 36, 15));
      try
        tCliDataNasc.Value := StrToDate(Trim(Copy(S, 65, 10)));
      except
      end;
      S := SL[I+6];
      tCliMinutos.Value := HMToMinutos(Copy(S, 17, 5));
