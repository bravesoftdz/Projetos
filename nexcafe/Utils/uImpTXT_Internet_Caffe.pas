...
{    if J=2 then begin
    ...
    s3 := UpperCase(ProxCampo);
    sDoc := SoDig2(s3);
    if (pos('rg', s3)=0) and (Length(sDoc)=11) then
      tCliCPF.Value := sDoc 
    else begin
      tCliRG.Value := SoDig2(s3);

      if S3>'' then
      if S3[Length(S3)] in ['A'..'Z'] then
        tCliRG.Value := tCliRG.Value + S3[Length(S3)];
    end;
