...
begin
  TFrmOpcoes.Create(Self).Editar(SOpçõesParaProdutos, [TFrmConfigAutoCad, TFrmConfigPrecoAuto, {$ifdef Loja}TFrmConfigQuickCad,{$endif}TFrmConfigComissao, TFrmConfigCodProdutoDuplicado]);
end;
...
begin
//  FVen:= nil;
  panLookup.Parent := nil;
  FMgr.Clear;
  Action:= CaFree;
end;

procedure TFrmProduto.FormShow(Sender: TObject);
var SaveIndex, i: Integer;
begin
...
begin
  if (not tCateg.FindKey([edCateg.EditValue])) and (edCateg.EditValue<>null) and (edCateg.EditValue<>'') then begin
    tCateg.Append;
    tCategDescricao.AsVariant := edCateg.EditValue;
    tCateg.Post;
  end;

  if tCateg.IsEmpty then begin
    tCateg.Append;
    tCateg.Post;
  end;
