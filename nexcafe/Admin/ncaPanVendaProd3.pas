...
type
  TFrmPanVendaProd2 = class(TFrmPanVendaProdBase)
    ...
    procedure imgPinCodClick(Sender: TObject);
    procedure imgPinQtdClick(Sender: TObject);
  private
    ...
    procedure SetComecarPor(const Value: Byte);
  ...
  public
    ...
    property ComecarPor: Byte
      read GetComecarPor write SetComecarPor;
...
begin
  ...
  if ComecarPor=0 then
    edProd.SetFocus else
    edQtd.SetFocus;
end;
...
{$ENDIF} 
  ...
  ComecarPor := StrToIntDef(slConfig.Values['ComecarPor'], 0);
...
function TFrmPanVendaProd2.GetComecarPor: Byte;
begin
  if imgPinCod.MouseOffDrawMode=idmNormal then
    Result := 0 else
    Result := 1;
end;

procedure TFrmPanVendaProd2.imgPinCodClick(Sender: TObject);
begin
  inherited;
  ComecarPor := 0;
  Self.SaveConfig;
...
procedure TFrmPanVendaProd2.imgPinQtdClick(Sender: TObject);
begin
  inherited;
  ComecarPor := 1;
  Self.SaveConfig;
...
procedure TFrmPanVendaProd2.SaveConfig;
begin
  slConfig.Values['ComecarPor'] := IntToStr(ComecarPor);
  ncaDM.SaveConfig;
end;
...
procedure TFrmPanVendaProd2.SetComecarPor(const Value: Byte);
begin
  if Value=0 then begin
    imgPinCod.MouseOffDrawMode := idmNormal;
    imgPinQtd.MouseOffDrawMode := idmFaded;
  end else
  begin
    imgPinCod.MouseOffDrawMode := idmFaded;
    imgPinQtd.MouseOffDrawMode := idmNormal;
  end;
end;
...
begin
  if edQtd.Focused then
    lbSize.Caption := edQtd.EditingText else
    lbSize.Caption := FloatToStr(FQuant);
  if edQtd.Focused then
    edQtd.Width := lbSize.Width + 10 else
    edQtd.Width := lbSize.Width;
  imgPinCod.Left := edProd.Left-3;  
end;
...
begin
  ...
  lbUnit.Visible := edUnit.Focused;
  lbUnit.Left := edUnit.Left + 3;
end;
