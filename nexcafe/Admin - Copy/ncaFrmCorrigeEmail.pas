...
const
  SHttpContasNextarComBr = 'http://contas.nextar.com.br';
...
begin
  ...
  if (Trim(FEmail)<>Trim(edEmail.Text)) then begin
    ...
    try
      ChaveSeg := H.Get(SHttpContasNextarComBr+'/obtemchaveseg'); // do not localize
      sl.Text := H.Get(SHttpContasNextarComBr+'/corrigiremail?email='+Trim(FEmail)+'&novoemail='+edEmail.Text+'&chaveseg='+ChaveSeg+'&senhaseg='+GeraSenhaSeg(ChaveSeg)); // do not localize
