...
uses
  ...
  cxImage, uParentedPanel, dxPScxPivotGridLnk, uNexTransResourceStrings_PT;
...
type
  TfbOpcoes = class(TFrmBase)
    ...
    panImpBobinaParams: TLMDSimplePanel;
...
uses 
  ncaDM, 
  ncaFrmPri, 
  ncaFrmFundo, uDMEmail, ncClassesBase, ncVersoes, ncafbCXLetra, ncaFrmCadCli,
  ufmImagens, Printers;
...
procedure TfbOpcoes.cbComboBoxPrintersPropertiesChange(Sender: TObject);
begin
  inherited;
...
procedure TfbOpcoes.rbImpBobinaClick(Sender: TObject);
begin
  inherited;
  if rbImpBobina.Enabled then
  if not MTRecMatricial.Value then begin
    if not (MT.State in [dsEdit, dsInsert]) then MT.Edit;
    MTRecMatricial.Value := True;
    rbImpBobina.Checked := True;
    AjustaVisRecibos;
  end;
end;

procedure TfbOpcoes.rbImpFolhaClick(Sender: TObject);
begin
  inherited;
  if rbImpFolha.Enabled then
  if MTRecMatricial.Value then begin
    if not (MT.State in [dsEdit, dsInsert]) then MT.Edit;
    MTRecMatricial.Value := False;
    rbImpFolha.Checked := True;
    AjustaVisRecibos;
  end;
end;
...
function TfbOpcoes.GetImprimirRec: Byte;
begin
...
procedure TfbOpcoes.SetImprimirRec(const Value: Byte);
begin
  case Value of
    0 : begin
      rbNao.Checked := True;
      rbNao.Font.Style := [fsUnderline];
      rbTalvez.Font.Style := [];
      rbSempre.Font.Style := [];
    end;  
    1 : begin
      rbTalvez.Checked := True;
      rbTalvez.Font.Style := [fsUnderline];
      rbNao.Font.Style := [];
      rbSempre.Font.Style := [];
    end;  
    2 : begin
      rbSempre.Checked := True;
      rbSempre.Font.Style := [fsUnderline];
      rbTalvez.Font.Style := [];
      rbNao.Font.Style := [];
    end;  
  end;
