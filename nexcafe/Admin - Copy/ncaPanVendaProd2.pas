...
begin
  ...
  if (edQtd.Value>tProEstoqueAtual.Value) and gConfig.NaoVenderAlemEstoque then
    if not TFrmSemEstoque.Create(Self).PodeVender(tProDescricao.Value, tProEstoqueAtual.Value) then Exit;
  ...
  if Popped and FPesqProd.cmCadRapido.Down then begin
    ...
    if Key=Key_Enter then begin
      if not Popped then begin
        ...
        if (FPesqProd.RecCount<>1) or FPesqProd.AchouPorDescr or FPesqProd.AchouParcial then begin
          Key := 0;
          Popped := True;
          Exit;
        end;
...
begin
  ...
  if edProd.Focused and (not FIgnoreChange) and Popped then
    FPesqProd.edBusca.EditValue := edProd.EditingText;
end;
...
begin
  if btnLancar.Enabled then begin
    if (TipoTran = trEstCompra) and (not edQtd.Focused) then
      edQtd.SetFocus
    else  
