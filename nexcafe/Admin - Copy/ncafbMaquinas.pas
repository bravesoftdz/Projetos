...
uses
  ...
  PngSpeedButton, cxContainer, cxLabel,
...
resourcestring
  ...
  SParaIniciarUmNovoAcessoNessaM·qu = 'Para iniciar um novo acesso nessa m·quina È necess·rio registrar o pagamento. ';
  SVocÍPoder·RegistrarOPagamentoAtr = 'VocÍ poder· registrar o pagamento atravÈs do duplo-clique ou do bot„o "Editar"';
  SJ·ExisteUmAcessoEmAndamentoNaM·q = 'J· existe um acesso em andamento na m·quina ';
  SAcessoGratuitoSÛPodeSerAbertoDir = 'Acesso gratuito sÛ pode ser aberto diretamente na m·quina cliente.';
  SEDespausar = '&E. Despausar';
  SEPausar = '&E. Pausar';
  SConfirmaTransferÍnciaDoAcessoDaM = 'Confirma transferÍncia do acesso da m·quina ';
  ...
  SEssaM·quinaN„oEst·ConectadaNoSer = 'Essa m·quina n„o est· conectada no servidor NexCafÈ. Clique para saber mais.';
...
{  if aFidResg then
  ...
  if TFrmTempo.Create(Self).Editar(True, True, @DT, 0, aAvulso, MinutosT, ValorT, aAvulso) then  
    try
...
begin
  ...
  if S<>nil then begin
    ...
    if S.Encerrou then 
      ShowMessage(SParaIniciarUmNovoAcessoNessaM·qu+
                  SVocÍPoder·RegistrarOPagamentoAtr)
    else
  ...
  try
    ...
    begin
      Beep;
      ShowMessage(SAcessoGratuitoSÛPodeSerAbertoDir);
      Exit;
    end;
...
begin
  ...
  with Dados, CM do
  if SimNao(SConfirmaTransferÍnciaDoAcessoDaM + 
...
{          if (xCell<CardWidth) and (xCell>(CardWidth-46)) then begin
    ...
    if M<>nil then begin
      ...
      case xCell of
        ...
        ibtnImpressao : 
          if M.Imp then begin
            FrmPri.MostraHint(P.X, P.Y, SImpressıes, SCliqueParaEditarCorrigirAImpress);
