...
//    LeaseExpires        : Integer;
  ...
  function StrToIp(addr:string):TIP;
  function checkIP(var addr:string):boolean;
  function StripIP(addr:string):string;
  function IPinNetwork(ip,nw:string):boolean;
...
begin
    ...
    if pos('.', addr)>0 then begin
        n := copy(addr, 1, pos('.', addr)-1);
        if isNumeric(n) and (strToint(n)>=0) and (strToint(n)<256) then begin
           raddr := n + '.';
        end else
           raddr := '0.';
        addr := copy(addr, pos('.', addr)+1, 15);
    ...
    if pos('.', addr)>0 then begin
        ...
        if pos('.', addr)>0 then begin
            ...
            if pos('.', addr)>0 then begin
                n := copy(addr, 1, pos('.', addr)-1);
                if isNumeric(n) and (strToint(n)>=0) and (strToint(n)<256) then begin
                   result.n3 := strtoint(n);
                end else
                   result.n3 := 0;
                n := copy(addr, pos('.', addr)+1, 15);
                if isNumeric(n) and (strToint(n)>=0) and (strToint(n)<256) then begin
                   result.n4 := strtoint(n);
                end else
                   result.n4 := 0;
...
begin
   result := false;
   if (checkIP(ip)) and (checkIP(nw)) then begin
       nip := StrToIp(ip);
       nnw := StrToIp(nw);
       result := ((nnw.n1=0 ) or (nnw.n1=nip.n1) ) and
                 ((nnw.n2=0 ) or (nnw.n2=nip.n2) ) and
                 ((nnw.n3=0 ) or (nnw.n3=nip.n3) ) and
                 ((nnw.n4=0 ) or (nnw.n4=nip.n4) );
   end;
end;
...
begin
  ...
  FDHCPServer.Free;
  FPrimaryWINSServer.Free;
  FSecondaryWINSServer.Free;
