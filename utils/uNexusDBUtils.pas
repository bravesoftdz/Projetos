...
//    function LeDateTime(aFieldName: String; aNew: Boolean=True): TDateTime;
    ...
    procedure SetDateTimeField(aFieldName: String; aValue: TDateTime; aData: PnxByteArray = nil);
    
    procedure ModifyNewData;
    procedure InsertNewData;
    procedure DeleteRec;
    function FindKey(aCount: Integer): Boolean;
    function FindNearest(aCount: Integer; aPreferLess: Boolean): Boolean;
    function SelIndex(aName: String): Boolean;
    function SetRange(aCount: Integer; aKeyExcl: Boolean=True): Boolean;
...
function TnxHelpCursor.FindNearest(aCount: Integer;
  aPreferLess: Boolean): Boolean;
var 
  I : Integer;  
  sa : TnxSearchKeyAction;

function GetRec: Boolean;
begin
  if sa = skaSmallerEqual then
    Result := (FCursor.RecordGetPrior(nxltNoLock, FNewData)=0) else
...
begin
  ...
  if aPreferLess then
    sa := skaSmallerEqual else
    sa := skaGreaterEqual;

  Result := (FCursor.SetToKey(sa, aCount, 0, False, FKeyData1)=0);
...
function TnxHelpCursor.Last: Boolean;
var erro: integer;
begin
  Result := (FCursor.SetToEnd=0);
  if Result then 
    Result := Prior;
end;

function TnxHelpCursor.LeBool(aFieldName: String; aNew: Boolean): Boolean;
var 
  aField: Integer;
  aData : PnxByteArray;
begin
  ...
  with FCursor.TableDescriptor, FieldsDescriptor do
...
function TnxHelpCursor.LeByte(aFieldName: String; aNew: Boolean): Byte;
var 
  aField: Integer;
  aData : PnxByteArray;
begin
...
function TnxHelpCursor.LeCurrency(aFieldName: String; aNew: Boolean): Currency;
var 
  aField: Integer;
  aData : PnxByteArray;
begin
...
{function TnxHelpCursor.LeDateTime(aFieldName: String; aNew: Boolean): TDateTime;
var 
  aField: Integer;
  aData : PnxByteArray;
begin
...
function TnxHelpCursor.LeDouble(aFieldName: String; aNew: Boolean): Double;
var 
  aField: Integer;
  aData : PnxByteArray;
begin
...
function TnxHelpCursor.LeInteger(aFieldName: String; aNew: Boolean): Integer;
var 
  aField: Integer;
  aData : PnxByteArray;
begin
...
function TnxHelpCursor.LeWord(aFieldName: String; aNew: Boolean): Word;
var 
  aField: Integer;
  aData : PnxByteArray;
begin
...
function TnxHelpCursor.LeWord32(aFieldName: String; aNew: Boolean): Cardinal;
var 
  aField: Integer;
  aData : PnxByteArray;
begin
...
function TnxHelpCursor.Next: Boolean;
begin
  Result := (FCursor.RecordGetNext(nxltNoLock, FNewData)=0);
end;
...
function TnxHelpCursor.Prior: Boolean;
begin
  Result := (FCursor.RecordGetPrior(nxltNoLock, FNewData)=0);
end;
...
procedure TnxHelpCursor.SetDateTimeField(aFieldName: String; aValue: TDateTime;
  aData: PnxByteArray = nil);
var C: Comp;  
begin
  if aData=nil then aData := FNewData;

  C := TimeStampToMSecs(DateTimeToTimeStamp(aValue));
  with FCursor.TableDescriptor, FieldsDescriptor do
    SetRecordField(GetFieldFromName(aFieldName), aData, @C);
end;
...
function TnxHelpCursor.SetRange(aCount: Integer; aKeyExcl: Boolean = True): Boolean;
begin
  Result := (FCursor.RangeSet(aCount, 0, False, FKeyData1, True, aCount, 0, False, FKeyData2, True)=0)
end;
